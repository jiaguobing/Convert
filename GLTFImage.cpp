...
GLTF::Image* GLTF::Image::load(std::string imagePath) {
	...
	size_t npos = imagePath.rfind('.');
	size_t lne = imagePath.length();
	std::string fileExtension = imagePath.substr(npos, lne - npos);
	fileExtension.erase(0, 1);
	GLTF::Image* image = NULL;
	FILE* file = fopen(imagePath.c_str(), "rb");
	if (file == NULL) {
		std::cout << "WARNING: Image uri: " << imagePath << " could not be resolved " << std::endl;
		image = new GLTF::Image(imagePath, imagePath);
	}
	else {
		...
		fclose(file);
		image = new GLTF::Image(imagePath, imagePath, buffer, bytesRead, fileExtension);
	}
