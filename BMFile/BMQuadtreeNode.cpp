...
void BMShellPrimitive::ComputeContent(float offsetX, float offsetY, float offsetZ)
{
...
void BMQuadtreeNode::CreateBMMesh(float offsetX, float offsetY, float offsetZ)
{
	...
	{
		_pShellPri->ComputeContent(offsetX,offsetY,offsetZ);
	}
	//////////////////////////////////////////////////////////////////////////
	...
	//将BMShellPrimitive 转换为 GLTF::BMPrimitive-----------------------------------------只处理颜色---未处理纹理
	for (auto _pShellPri : m_vShellPrimitives)
	{
		GLTF::BMPrimitive _bmPri;
		_bmPri.m_vBatchIDs.reserve(_pShellPri->m_vPositions.size());
		_bmPri.m_vNormals.reserve(_pShellPri->m_vPositions.size());
		_bmPri.m_vPositions.reserve(_pShellPri->m_vPositions.size());
		//
		_bmPri.m_vPositions.insert(_bmPri.m_vPositions.end(), _pShellPri->m_vPositions.begin(), _pShellPri->m_vPositions.end());
		_bmPri.m_vNormals.insert(_bmPri.m_vNormals.end(), _pShellPri->m_vNormals.begin(), _pShellPri->m_vNormals.end());
		//
...
void BMQuadtreeManager::ParseSegment(HC_KEY segmentKey)
{
	...
	for (auto key : _vSegments)
	{
		...
		if (_hasMinMax)
		{
			...
			//
			float _BoxLen = max(_maxPt.x - _minPt.x, _maxPt.y - _minPt.y);
